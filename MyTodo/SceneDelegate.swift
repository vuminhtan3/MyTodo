...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let isCompletedInputName = UserDefaultsService.shared.completedInputName
        
        if isCompletedInputName {
            routeToMain()
        } else {
            routeToStart()
        }
    ...
    private func routeToStart() {
        let mainStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let startVC = mainStoryboard.instantiateViewController(withIdentifier: "StartViewController")
